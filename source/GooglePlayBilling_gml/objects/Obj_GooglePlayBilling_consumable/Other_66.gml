...
switch(eventId)
{
	case gpb_product_consume_response:	
		...
		// Check if purchase tokens match
		if(responseData.purchaseToken == purchaseToken)
		{
			// At this point we know the event refers to this in-app purchase.
			// Now we need to check if the operation succeeded
			if (responseData.success) 
			{
				// At this point the purchase consumption succeeded.
				// We can now call the event/function responsible for handling the OnConsumed logic.
				event_user(1);
			}
			else
			{
				// At this point the purchase consumption failed.
				//  
				...
				//
				// We can now call the event/function responsible for handling the OnConsumeFailed logic.
				event_user(2);
			}
		}
		break;
